 ...
 *  
 *  Conveniently, the relationship is reversible - every node's parent is at floor((c-1)/2).
 */
class ReferenceArrayBinaryHeap {
   ...
   *  and so on, and so forth, until we get to the root node which actually has log n maximum swaps,
   ...
   *  the heap until it satisfies the heap property.
   *  
   *  @timeComplexity O(log n)
   *  @spaceComplexity O(1)
   */
  _heapUp(n) {
    ...
    // Only heap up if n greater than 0, otherwise parent will be negative / out of bounds
    if (n > 0) {
  ...
  _heapDown(n) {
    ...
    // Only heap down if n has child(ren)
    if (leftChild < this._size) {

      // Check for right child and update minChild if right is less than left
      if (rightChild < this._size && this._comparator(this._storage[rightChild], this._storage[leftChild]) < 0) {
        minChild = rightChild;
      }

      if (this._comparator(this._storage[minChild], this._storage[n]) < 0) {
        this._swap(n, minChild);
        this._heapDown(minChild);
      }
    }
  }
}
